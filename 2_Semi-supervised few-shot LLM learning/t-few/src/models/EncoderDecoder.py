...
class EncoderDecoder(LightningModule):
    ...
    def predict(self, batch):
        ...
        # Save batch_output to a CSV file
        csv_file_path = f"D1D2D3_pseudo_num_shot_{self.config.data_name}_seed_{self.config.seed}_prob_{self.config.data_prob}.csv"
        fieldnames = batch_output.keys()
    ...
    def save_model(self, finish=False):
        if self.config.save_model and (finish or self._last_global_step_saved != self.global_step):
            if finish:
                ...
                model_fname = os.path.join(self.config.exp_dir, 
                            f"{self.config.num_shot}fewshot_{self.config.seed}seed_global_step{self.global_step}.pt")
